...
using HTTPServerLib;
using PrintServer2.Common;
using PrintServer2.Template;
...
namespace PrintServer2.Server
{
    public class PrintServer
    {
        ...
        private string printEngin = null;
        ...
        public PrintServer()
        {
            ...
            this.printEngin = AppSettingHelper.GetOne("engin", "PDF");
        ...
        private void PrintWithThread(object objectPara)
        {
            try
            {
                ...
                if (httpRequest != null)
                {
                    IPrintObject model = PrintObjectFactory.GetPrintModel(this.printEngin, httpRequest.Body);
                    if (model.Intervel() <= 0)
                    {
                        //不要求顺序打印
